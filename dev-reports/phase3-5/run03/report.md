...
| 文件 | 说明 |
|------|------|
| `public/cards/sanitized/main_paimon-sanitized.json` | 派蒙清洗版（v1） |
| `public/cards/sanitized/main_ganyu-sanitized.json` | 甘雨清洗版（v1） |

两张卡均使用 `character_version: "sanitized-v1"`，可与原始卡共存，通过 `cards-manifest.json` 选择加载。
...
在 `cards-manifest.json` 中将清洗版路径替换原始路径，即可加载清洗卡。例如：

```json
[
  "sanitized/main_paimon-sanitized.json",
  "sanitized/main_ganyu-sanitized.json"
]
```
