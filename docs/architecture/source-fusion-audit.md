...
| MCP-driven expression commands from LLM | `src/services/character/character-service.ts`, `src/services/character/expression-protocol.ts`, `src/features/stage/StageWindow.tsx`, `src/main.tsx` | `partial` | A first-pass reusable emotion protocol now exists with a small distinct taxonomy and randomized per-model expression candidates, but motion selection and full behavioral validation are still pending. | Continue `P2.3` with motion wiring and validation. |
