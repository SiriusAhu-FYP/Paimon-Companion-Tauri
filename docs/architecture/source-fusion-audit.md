...
| Async VLM client abstraction | `src/services/vlm/openai-compatible-vision-client.ts`, `src/services/games/game-2048-service.ts`, `src/services/games/sokoban-service.ts`, `src/services/companion-runtime/companion-runtime-service.ts` | `partial` | The Tauri app now has a shared OpenAI-compatible vision client used by both gameplay reasoning and the companion runtime. It is still intentionally smaller than the MVP’s broader toolkit surface. | Keep this shared client thin and only extend it when a real runtime caller needs more than image reasoning. |
...
| Queue manager / backpressure / expiry around frame processing | `src/services/companion-runtime/companion-runtime-service.ts` | `partial` | The runtime now has rolling retention, lightweight unchanged-frame filtering, bounded queue pruning, and self-paced capture/summary scheduling to avoid overlap drift. It still stops well short of the MVP’s fuller backend pipeline/scheduler toolkit. | Keep strengthening only the queue behavior that the product runtime actually needs. |
...
- when the companion runtime is already observing the same target, unified rounds now refresh that observation context before generating the follow-up reply
