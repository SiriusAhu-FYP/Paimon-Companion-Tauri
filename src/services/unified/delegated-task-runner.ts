...
const PROGRESS_EVALUATOR_TEXT_MAX_TOKENS = 2000;
const PROGRESS_EVALUATOR_VISION_MAX_TOKENS = 2000;
...
export async function runDelegatedTaskLoop(input: {
	...
	for (let round = 1; round <= config.maxRounds; round += 1) {
		...
		reflection = enrichEvaluatorDiagnosisFromExecutionError(
			reflection,
...
function enrichEvaluatorDiagnosisFromExecutionError(
	reflection: ProgressEvaluatorDecision,
	executionError: string,
	planner: OperationsPlannerDecision,
	actionSummary: string,
	...
	const routeLearning = [
		...
		`failureGeometry=${failureGeometry}`,
		`routeLesson=${routeLesson}`,
		`nextAttemptConstraint=${nextAttemptConstraint}`,
		planner.committedRoute ? `preserveUsefulPrefixFrom=${planner.committedRoute}` : "",
		failedPrefix ? `abandonUnchangedPrefix=${failedPrefix}` : "",
	].filter(Boolean).join(" | ");
	return {
		...reflection,
